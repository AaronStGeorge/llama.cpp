...
// P058: VECTORIZED (float4) split-K decode flash attention for head-dim 128, GQA-agnostic (one query head per
// row_group / wavefront). Adapted from hrx_flash_attn_ext_f32_f16_decode_gqa8_split (the D=256/GQA-8 kernel):
// identical float4 QK dot + subgroup reductions + register-resident online softmax, retargeted to D=128 /
// GQA-4 (VEC_PER_THREAD 8->4; one head per row_group instead of two). Writes un-normalized (O, m, l) partials
// to scratch; hrx_flash_attn_ext_f32_f16_decode_combine reduces across SPLITS. Grid {H_KV*SPLITS, N, S}, 128
// threads. Replaces the earlier scalar split kernel (which floored at 723 us/tok vs HIP's float4 294).
...
// Reduce across the d_tid dimension (D_SPLIT=8 lanes -> masks 4,2,1). Unchanged from gqa8 (D_SPLIT preserved).
...
// Reduce across the col_tid dimension (col_tid in [0,4) -> masks 8,16). Unchanged from gqa8.
