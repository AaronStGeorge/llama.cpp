...
const loomc_artifact_t * ggml_hrx_loom_jit_find_artifact(const loomc_result_t * result,
    ...
    for (loomc_host_size_t i = 0; i < loomc_result_artifact_count(result); ++i) {
        ...
        const bool role_matches     = loomc_string_view_equal(artifact->role, role);
        const bool format_matches   = loomc_string_view_equal(artifact->format, format);
        const bool artifact_matches = role_matches && format_matches;
        if (artifact_matches) {
            return artifact;
        }
    }
...
hrx_status_t ggml_hrx_loom_jit_amdgpu_compile(ggml_hrx_loom_jit_amdgpu *                jit,
    ...
    const loomc_artifact_t * compile_report = ggml_hrx_loom_jit_find_artifact(
        result.get(), loomc_make_cstring_view(LOOMC_ARTIFACT_ROLE_COMPILE_REPORT),
    ...
    const loomc_artifact_t * final_module = ggml_hrx_loom_jit_find_artifact(
        result.get(), loomc_make_cstring_view(LOOMC_ARTIFACT_ROLE_MODULE),
    ...
    if (hrx_status_is_ok(hrx_status)) {
        const loomc_artifact_t * manifest =
            ggml_hrx_loom_jit_find_artifact(result.get(),
